...
const Sidebar2 = ({ isOpen, toggleSidebar }) => {
  return (
    <aside className="">
      ...
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 w-64 p-3 min-h-screen transform transition-transform duration-300 z-30 ${
          isOpen ? "translate-x-0" : "-translate-x-full"
        } md:translate-x-0 md:static md:w-64`}
      >
        {/* Profile Actions */}
        <div className="p-2 rounded-md">
          <div className="flex items-center bg-[#4aa0ba] text-white font-bold p-2 rounded-md">
          ...
          <ul className="mt-2 text-xs space-y-1">
            {[
              ...
              <li
                key={index}
                className="border-b border-gray-300 py-1 px-2 hover:bg-[#4aa0ba] cursor-pointer rounded-md"
              >
                {item}
              </li>
            ))}
        ...
        <div className="p-2 rounded-md mt-4">
          ...
          <div className="text-xs">
            ...
            <button className="w-full mt-2 bg-[#004054] text-white py-1 rounded-md hover:bg-[#00333b] text-xs">
